...
struct PhotosView: View {
    ...
    private var areaTitle: String { packStore.allPhotoAreas.first { $0.id == selectedArea }?.title ?? "" }
    ...
    var body: some View {
                    ...
                    ScrollView(.horizontal, showsIndicators: false) {
                        HStack(spacing: 8) {
                            ForEach(packStore.allPhotoAreas) { area in
                                let selected = selectedArea == area.id
                                Button { selectedArea = area.id } label: {
                                    VStack(spacing: 2) {
                                        Text(area.emoji)
        ...
        .onAppear {
            if selectedArea.isEmpty || !packStore.allPhotoAreas.contains(where: { $0.id == selectedArea }) {
                selectedArea = packStore.allPhotoAreas.first?.id ?? "progress"
            }
        }
        ...
        .sheet(isPresented: $showTimelapse) {
            if let area = packStore.allPhotoAreas.first(where: { $0.id == selectedArea }) {
...
struct TimelapseView: View {
    ...
    private func load() {
        photos = store.photos(for: area.id).sorted { $0.dateKey < $1.dateKey }
