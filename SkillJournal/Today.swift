...
struct RootView: View {
    ...
    enum Tab: Int {
        case today = 0, explore, photos, reminders
    }
    ...
    var body: some View {
        ...
        .onReceive(NotificationCenter.default.publisher(for: .openExploreTab)) { _ in
            selectedTab = .explore
        }
    }
}
...
/// the wishlist, which the display-only `ProductInfo` doesn't carry.
...
struct TodayView: View {
    ...
    /// First-run welcome — the Explore tab is the onboarding entry; the
    /// manual path opens the editor.
    private var emptyState: some View {
        VStack(spacing: 10) {
            Text("📓")
                .font(.system(size: 44))
            Text("Welcome to Skill Journal")
                .font(.system(.title2, design: .serif).weight(.semibold))
                .foregroundColor(.ink)
            Text("Pick something to get better at, build a routine, check it off daily.\nEverything stays on your phone — no account, no tracking.")
                .font(.footnote)
                .foregroundColor(.soft)
                .multilineTextAlignment(.center)
            Button {
                NotificationCenter.default.post(name: .openExploreTab, object: nil)
            } label: {
                Text("Choose what to get better at ✨")
            ...
            .padding(.top, 2)
            Text("Guidance only — not professional advice.")
                .font(.caption2)
    ...
    /// non-daily step get the "rests today" note on off days.
    private func dynamicFooter(for routine: Routine) -> String? {
        if let footer = routine.footer { return footer }
        let resting = routine.steps.contains { step in
            guard let cadence = step.cadence, cadence.kind != "daily" else { return false }
            return !cadence.isActive(on: store.selectedDate)
        }
        if resting {
            return "Some steps rest today — back soon ✨"
        }
        return nil
    }
}
    ...
    var body: some View {
        HStack(spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(Calendar.current.isDateInToday(store.selectedDate) ? "Hello ♡" : dayName)
                    .font(.system(.title2, design: .serif).weight(.semibold))
                    .foregroundColor(.ink)
                Text("\(dayName), \(dateLine)\(store.todayFocus.map { " · \($0)" } ?? "")")
                    .font(.footnote).foregroundColor(.soft)
                Text("\(store.doneCount) of \(store.allKeys.count) done — keep it up ✨")
            ...
            .accessibilityLabel("About Skill Journal")
        }
        .padding(.top, 8)
...
struct StepRowView: View {
    ...
    private var product: ProductInfo? { step.productID.flatMap { store.productInfo($0) } }
...
struct ProductSheet: View {
    ...
    private var product: ProductInfo { selection.product }
    ...
    private var actionFooter: some View {
        HStack(spacing: 8) {
            if store.packs.product(selection.productID) != nil {
